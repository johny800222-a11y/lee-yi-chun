...
# ── v2 強化：EMA99 斜率過濾（空單需均線向下）────────────────────
EMA_SLOPE_BARS  = 5      # 用前 N 根判斷 EMA99 斜率
SHORT_EMA_SLOPE_THRESHOLD = -0.0002  # EMA99 每根K棒跌幅（相對比例）必須 < 此值
    ...
    if idx >= EMA_SLOPE_BARS:
        ...
        if sma_prev > 0:
            ...
            if slope > SHORT_EMA_SLOPE_THRESHOLD:
                log.info(f"[SLOPE_FILTER] short 跳過：EMA99 斜率={slope:.5f} > {SHORT_EMA_SLOPE_THRESHOLD}（均線未向下）")
                return None
